...
#ifndef SBK_QFSFILEENGINEWRAPPER_H
#define SBK_QFSFILEENGINEWRAPPER_H
...
class QFSFileEngineWrapper : public QFSFileEngine
{
public:
    QFSFileEngineWrapper();
    QFSFileEngineWrapper(const QString & file);
    ...
    virtual bool seek(qint64 arg__1);
    ...
    virtual ~QFSFileEngineWrapper();
    // Inherited overloads, because the using keyword sux
    inline bool open(int flags, int fd) { return QFSFileEngine::open(QFlags<QIODevice::OpenModeFlag>(flags), fd); }
    inline bool open(int flags, int fd, int handleFlags) { return QFSFileEngine::open(QFlags<QIODevice::OpenModeFlag>(flags), fd, QFlags<QFile::FileHandleFlag>(handleFlags)); }
    static void pysideInitQtMetaTypes();
};

#endif // SBK_QFSFILEENGINEWRAPPER_H
