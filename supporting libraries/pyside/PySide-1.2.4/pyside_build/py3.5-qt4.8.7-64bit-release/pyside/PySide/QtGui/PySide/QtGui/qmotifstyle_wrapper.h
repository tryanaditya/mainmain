...
#ifndef SBK_QMOTIFSTYLEWRAPPER_H
#define SBK_QMOTIFSTYLEWRAPPER_H
...
class QMotifStyleWrapper : public QMotifStyle
{
public:
    QMotifStyleWrapper(bool useHighlightCols = false);
    ...
    virtual ~QMotifStyleWrapper();
public:
    virtual int qt_metacall(QMetaObject::Call call, int id, void** args);
    virtual void* qt_metacast(const char* _clname);
    // Inherited overloads, because the using keyword sux
    inline QPixmap standardPixmap(int standardPixmap, const QStyleOption * opt, const QWidget * widget = 0) const { return QMotifStyle::standardPixmap(QStyle::StandardPixmap(standardPixmap), opt, widget); }
    static void pysideInitQtMetaTypes();
};

#endif // SBK_QMOTIFSTYLEWRAPPER_H
